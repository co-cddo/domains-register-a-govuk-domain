...
def get_notification_for_id(
    ...
    try:
        ...
        if e.response.status_code == 404 and e.response.reason == "NOT FOUND":
            ...
            """
            logger.info(
                f"Notification with id {notification_response_id.id} not found."
            )
...
def check_email_failure_and_notify() -> None:
    ...
    TODO: Once celery has been added, this function should be moved as shared_task to be run periodically
    and should not be kept here.
    """
    ...
    if notifications_client:
        ...
        for notification_response_id in notification_response_ids:
            notification = get_notification_for_id(
                notifications_client, notification_response_id
            )
            if notification:
                ...
                if "failure" in notification["status"]:
                    notification["application_reference"] = reference(
                        notification["body"]
                    )
                    logger.info(
                        f"Email send for Application reference {notification['application_reference']} failed"
                    )
...
class Command(BaseCommand):
    help = """Check Notify email failure to registrar email address, if failed then update application status and
    send notification to internal team"""
