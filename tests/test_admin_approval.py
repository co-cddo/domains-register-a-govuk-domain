...
class ModelAdminTestCase(AdminScreenTestMixin, TestCase):
    ...
    def test_change_status_works(self, status):
        """
        Test case to check that we load the correct application on the review screen.
        When we click on change status, we confirm the status change of the application.
        :return:
        """
        request = Mock()
        request.session = SessionDict({"registration_data": self.registration_data})
        ...
        with self.subTest("Approving the review brings up the correct data"):
            ...
            self.assertContains(
                approve_response,
                "Send an email confirming the change_status to dummy_registrar_email-xx@gov.uk",
            )
            self.assertContains(
                approve_response, "Reason for change_status: change_status"
            )

            # Make sure the id displayed on the url is the id if the application
            self.assertEqual(
                f"/admin/application_confirm/?obj_id={application_to_approve.id}&action=change_status",
                approve_response.redirect_chain[0][0],
            )
        ...
        with self.subTest(
            ...
            # Refresh from the database
            application_to_approve.refresh_from_db()
            print(f"Status is set to {application_to_approve.status}")

            self.assertEqual(
                f"{status}",
                application_to_approve.status,
            )
            self.assertContains(approve_response, "Change_status email sent")
            self.assertEqual(
                application_to_approve.last_updated_by.username, "superuser"
            )
